...
| `tqt3-patches/` | 对 tqt3 的补丁（构建时打入 `tqt3-build/`；当前 4 个：qstring 的 TQString==const char* 遵循 codecForCStrings 语义修复 UTF-8 中文比较、启用内建 GIF 编解码、弹窗去除 save-under、Qt1 兼容插入重载遵循 codec；数量与清单以 `tqt3-patches/README.md` 为准） |
