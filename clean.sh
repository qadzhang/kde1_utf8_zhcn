...
#
# ┌─ What : 清理全部 7 个模块的构建产物（各模块 build/ 目录）与统一安装暂存区（staging/）
...
#
# 伪代码：
#   1. 模块清单 = qt1 kdelibs kdebase kdegames kdeutils kdenetwork kdetoys
#      （与 build.sh 主循环的清单逐项一致，任何一侧增删模块必须同步另一侧）
#   2. 逐个删除 <模块>/build
#   3. 删除根下 staging/（build.sh 的 DESTDIR 暂存区）
