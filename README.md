...
我第一次接触 Linux 时还年轻，用的就是 KDE 1。那时便被这套比 Windows 98 还要漂亮的界面深深吸引——它是我 Linux 世界的起点，也成了心里的那道"白月光"。此后时代奔涌向前，无数开发者接续耕耘，开源软件的功能与易用性与日俱增；而 KDE 1 却渐渐从各个发行版里消失，也从我的系统里消失了。每每想起，仍略带伤感。

后来，AI 时代到来。我从 GLM-4.7 一路用到今天，看着模型的能力一步步逼近"能与人协作完成真正工程"的水平，复活白月光终于从怀念变成了可能。但对我而言，**复活不是回到过去，而是把它带到现代**——不是在模拟器里把功能跑起来就算完，而是换上现代的底座与库：TQt3、fontconfig/Xft、fcitx5、WebKit2GTK、PipeWire、CUPS……让这位白月光在今天的系统上重新发光，并且第一次真正说好中文。
...
> 本项目发布版本：**1.2.0 RC1**（复活版版本号，区别于上游 1.1.2；Debian 包版本 `1.2.0~rc1-1`）。
