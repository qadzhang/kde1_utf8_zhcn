 ...
 * 拉入的 TQt3 头（ntqvariant.h/ntqmovie.h）。kwm 源码的 Bool 全局声明已
 * 全部 bool 化、不再依赖宏，在此摘除以保护后续 TQt3 头；X11 函数声明
 * 已于上方解析完毕。 */
#undef Bool
#undef Status
...
// the Client class encapsulates a window decoration. There磗 one
...
protected:
  // Some of Qt磗 syntactic events with which we will have to deal
