...
extern Atom tqt_net_wm_state_max_v;
extern Atom tqt_net_wm_state_max_h;
extern Atom tqt_net_wm_state_fullscreen;
extern Atom tqt_net_wm_state_above;
extern Atom tqt_net_wm_state_stays_on_top;
extern Atom tqt_net_wm_action;
extern Atom tqt_net_wm_action_move;
extern Atom tqt_net_wm_action_resize;
extern Atom tqt_net_wm_action_minimize;
extern Atom tqt_net_wm_action_shade;
extern Atom tqt_net_wm_action_stick;
extern Atom tqt_net_wm_action_max_h;
extern Atom tqt_net_wm_action_max_v;
extern Atom tqt_net_wm_action_fullscreen;
extern Atom tqt_net_wm_action_change_desktop;
extern Atom tqt_net_wm_action_close;
extern Atom tqt_net_wm_action_above;
extern Atom tqt_net_wm_action_below;
extern Atom tqt_net_wm_window_type;
extern Atom tqt_net_wm_window_type_normal;
extern Atom tqt_net_wm_window_type_dialog;
extern Atom tqt_net_wm_window_type_toolbar;
extern Atom tqt_net_wm_window_type_menu;
extern Atom tqt_net_wm_window_type_utility;
extern Atom tqt_net_wm_window_type_splash;
extern Atom tqt_net_wm_window_type_override;
extern Atom tqt_net_wm_window_type_dropdown_menu;
extern Atom tqt_net_wm_window_type_popup_menu;
extern Atom tqt_net_wm_window_type_combo;
extern Atom tqt_net_wm_window_type_dnd;
...
extern unsigned long *tqt_net_virtual_root_list;
...
Window tqt_XCreateWindow( const TQWidget *creator,
			 Display *display, Window parent,
			 int x, int y, uint w, uint h,
			 int borderwidth, int depth,
			 uint windowclass, Visual *visual,
			 ulong valuemask, XSetWindowAttributes *attributes );
Window tqt_XCreateSimpleWindow( const TQWidget *creator,
			       Display *display, Window parent,
			       int x, int y, uint w, uint h, int borderwidth,
			       ulong border, ulong background );
void tqt_XDestroyWindow( const TQWidget *destroyer,
...
static void tqt_net_change_wm_state(const TQWidget* w, bool set, Atom one, Atom two = 0)
{
    if (w->isShown()) {
	// managed by WM
        XEvent e;
        e.xclient.type = ClientMessage;
        e.xclient.message_type = tqt_net_wm_state;
	...
	if (j)
	    XChangeProperty(w->x11Display(), w->winId(), tqt_net_wm_state, XA_ATOM, 32,
			    PropModeReplace, (uchar *) new_states, j);
	else
	    XDeleteProperty(w->x11Display(), w->winId(), tqt_net_wm_state);
...
void TQWidget::create( WId window, bool initializeWindow, bool destroyOldWindow)
{
	...
	if ( x11DefaultVisual() && x11DefaultColormap() ) {
	    id = (WId)tqt_XCreateSimpleWindow( this, dpy, parentw,
	    ...
	    id = (WId)tqt_XCreateWindow( this, dpy, parentw,
					crect.left(), crect.top(),
					crect.width(), crect.height(),
    ...
    net_winactions[curr_winaction++] = tqt_net_wm_action_stick;
    net_winactions[curr_winaction++] = tqt_net_wm_action_fullscreen;
    net_winactions[curr_winaction++] = tqt_net_wm_action_change_desktop;
    net_winactions[curr_winaction++] = tqt_net_wm_action_above;
    net_winactions[curr_winaction++] = tqt_net_wm_action_below;
    ...
    if ( testNFlags(WX11DisableResize) ) {
        ...
        net_winactions[curr_winaction++] = tqt_net_wm_action_resize;
    }
    ...
    if ( testNFlags(WX11DisableMove) ) {
        ...
        net_winactions[curr_winaction++] = tqt_net_wm_action_move;
    }
    ...
    if ( testNFlags(WX11DisableMinimize) ) {
        ...
        net_winactions[curr_winaction++] = tqt_net_wm_action_minimize;
    }
    ...
    if ( testNFlags(WX11DisableMaximize) ) {
        ...
        net_winactions[curr_winaction++] = tqt_net_wm_action_max_h;
        net_winactions[curr_winaction++] = tqt_net_wm_action_max_v;
    }
    ...
    if ( testNFlags(WX11DisableClose) ) {
        ...
        net_winactions[curr_winaction++] = tqt_net_wm_action_close;
    }

    if ( ! testNFlags(WX11DisableShade) ) {
        net_winactions[curr_winaction++] = tqt_net_wm_action_shade;
    }
	...
	if ( testWFlags(WStyle_Splash) ) {
            if (tqt_net_supports(tqt_net_wm_window_type_splash)) {
	...
	if (testWFlags(WStyle_Customize)) {
		...
		net_winstates[curr_winstate++] = tqt_net_wm_state_max_v;
		net_winstates[curr_winstate++] = tqt_net_wm_state_max_h;
	    }
	}

	// stays on top
	if (testWFlags(WStyle_StaysOnTop)) {
	    net_winstates[curr_winstate++] = tqt_net_wm_state_above;
	    net_winstates[curr_winstate++] = tqt_net_wm_state_stays_on_top;
	}
        ...
        if (testWFlags(WShowModal)) {
            ...
            net_winstates[curr_winstate++] = tqt_net_wm_state_modal;
        }
	...
	protocols[n++] = tqt_net_wm_ping;	// support _NET_WM_PING protocol
#ifndef TQT_NO_XSYNC
	protocols[n++] = tqt_net_wm_sync_request;// support the _NET_WM_SYNC_REQUEST protocol
#endif
	if ( testWFlags( WStyle_ContextHelp ) )
	    protocols[n++] = tqt_net_wm_context_help;
	...
	if (curr_winstate > 0)
	    XChangeProperty(dpy, id, tqt_net_wm_state, XA_ATOM, 32, PropModeReplace,
            ...
            XDeleteProperty(dpy, id, tqt_net_wm_state);
	...
	if (curr_winaction > 0)
	    XChangeProperty(dpy, id, tqt_net_wm_action, XA_ATOM, 32, PropModeReplace,
            ...
            XDeleteProperty(dpy, id, tqt_net_wm_action);
    ...
    if ( destroyw )
	tqt_XDestroyWindow( this, dpy, destroyw );
...
void TQWidget::destroy( bool destroyWindow, bool destroySubWindows )
{
	...
	if ( testWFlags(WType_Desktop) ) {
	    ...
	    if ( destroyWindow )
		tqt_XDestroyWindow( this, x11Display(), winid );
	}
#ifndef TQT_NO_XSYNC
        destroySyncCounter();
#endif
...
void TQWidget::reparentSys( TQWidget *parent, WFlags f, const TQPoint &p, bool showIt )
{
	...
	show();
    if ( old_winid )
	tqt_XDestroyWindow( this, dpy, old_winid );
    if ( setcurs )
	setCursor(oldcurs);
...
void TQWidget::x11SetWindowType( X11WindowType type )
{
    ...
    if( type == X11WindowTypeSelect ) {
        if ( testWFlags(WStyle_Splash)) {
            if (tqt_net_supports(tqt_net_wm_window_type_splash)) {
                net_wintypes[curr_wintype++] = tqt_net_wm_window_type_splash;
            }
        } else if (inherits("TQToolBar")) {
	    // toolbar netwm type
	    net_wintypes[curr_wintype++] = tqt_net_wm_window_type_toolbar;
        } else if (testWFlags(WStyle_Customize) && testWFlags(WStyle_Tool)) {
	    // utility netwm type
	    net_wintypes[curr_wintype++] = tqt_net_wm_window_type_utility;
        } else if (testWFlags(WType_Dialog)) {
            // dialog netwm type
        ...
        // combo netwm type
	net_wintypes[curr_wintype++] = tqt_net_wm_window_type_combo;
    } else if( type == X11WindowTypeDND ) {
        // dnd netwm type
    	net_wintypes[curr_wintype++] = tqt_net_wm_window_type_dnd;
    } else if( type == X11WindowTypeDropdown ) {
        // dropdown netwm type
    	net_wintypes[curr_wintype++] = tqt_net_wm_window_type_dropdown_menu;
    } else if( type == X11WindowTypePopup ) {
        // popup netwm type
    	net_wintypes[curr_wintype++] = tqt_net_wm_window_type_popup_menu;
    } else if( type == X11WindowTypeMenu ) {
        // menu netwm type
	net_wintypes[curr_wintype++] = tqt_net_wm_window_type_menu;
    } else if( type == X11WindowTypeTooltip ) {
        // tooltip netwm type
    	net_wintypes[curr_wintype++] = tqt_net_wm_window_type_tooltip;
    }
    ...
    if (curr_wintype > 0)
        ...
        XDeleteProperty(x11Display(), winId(), tqt_net_wm_window_type);
}
...
void TQWidget::setWindowState(uint newstate)
{
	...
	if ((oldstate & WindowMaximized) != (newstate & WindowMaximized)) {
	    if (tqt_net_supports(tqt_net_wm_state_max_h) && tqt_net_supports(tqt_net_wm_state_max_v)) {
		tqt_net_change_wm_state(this, (newstate & WindowMaximized),
				       tqt_net_wm_state_max_h, tqt_net_wm_state_max_v);
	...
	if ((oldstate & WindowFullScreen) != (newstate & WindowFullScreen)) {
		...
		tqt_net_change_wm_state(this, (newstate & WindowFullScreen),
				       tqt_net_wm_state_fullscreen);
	    } else {
                needShow = isVisible();
...
void TQWidget::showWindow()
{
	...
	if (tqt_x_user_time != CurrentTime) {
	    XChangeProperty(x11Display(), winId(), tqt_net_wm_user_time, XA_CARDINAL,
	...
	if (!topData()->embedded &&
 	...
 	if (isMaximized() && !isFullScreen()
	    && !(tqt_net_supports(tqt_net_wm_state_max_h)
		 && tqt_net_supports(tqt_net_wm_state_max_v))) {
 	    XMapWindow( x11Display(), winId() );
	...
	if (isFullScreen() && !tqt_net_supports(tqt_net_wm_state_fullscreen)) {
 	    XMapWindow(x11Display(), winId());
 	    tqt_wait_for_window_manager(this);
	    return;
	}
...
{
    extern void tqt_erase_rect( TQWidget*, const TQRect& ); // in qpainer_x11.cpp
    if ( w < 0 )
	w = crect.width()  - x;
    if ( h < 0 )
	h = crect.height() - y;
    if ( w != 0 && h != 0 )
	tqt_erase_rect( this, TQRect(x, y, w, h ) );
}
...
{
    extern void tqt_erase_region( TQWidget*, const TQRegion& ); // in qpainer_x11.cpp
    tqt_erase_region( this, reg );
}
...
void TQWidget::updateFrameStrut() const
{
	...
	if (p == r ||
	    ...
	    break;
	} else if (tqt_net_supports(tqt_net_virtual_roots) && tqt_net_virtual_root_list) {
	    int i = 0;
	    while (tqt_net_virtual_root_list[i] != 0) {
		if (tqt_net_virtual_root_list[i++] == p)
		    break;
	    }
	}
