...
static DndTargetsTable TargetsTable(Display *display)
{
	...
	type == None) {
	tqWarning("TQMotifDND: cannot get property on motif window");
	return 0;
    }

    if (target_prop->protocol_version != DND_PROTOCOL_VERSION) {
	tqWarning("TQMotifDND: protocol mismatch");
    }
...
TQByteArray tqt_motifdnd_obtain_data( const char *mimeType )
{
    ...
    XConvertSelection (tqt_xdisplay(), Dnd_selection, conversion_type,
    ...
    XConvertSelection (tqt_xdisplay(), Dnd_selection, Dnd_transfer_success,
		       Dnd_selection, tw->winId(), Dnd_selection_time);
...
void tqt_motifdnd_handle_msg( TQWidget * /* w */ , const XEvent * xe, bool /* passive */ )
{
	...
	if (!in_drop_site) {
	    ...
	    XConvertSelection (tqt_xdisplay(), dnd_data.property, Dnd_transfer_failure,
