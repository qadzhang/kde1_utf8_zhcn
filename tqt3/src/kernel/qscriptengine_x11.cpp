...
static bool syriac_shape(TQShaperItem *item)
{
...
static bool thaana_shape(TQShaperItem *item)
{
...
static bool indic_shape_syllable(TQOpenType *openType, TQShaperItem *item, bool invalid)
{
    Q_UNUSED(openType)
    ...
    if (invalid) {
        *reordered.data() = 0x25cc;
        memcpy(reordered.data()+1, item->string->unicode() + item->from, len*sizeof(TQChar));
        len++;
    } else {
        memcpy(reordered.data(), item->string->unicode() + item->from, len*sizeof(TQChar));
    }
    ...
    *invalid = false;
    ...
    while (pos < end - start) {
        ...
        case Control:
 	    ...
 	    if (state == Halant && uc[pos].unicode() == 0x200d /* ZWJ */)
  		break;
            ...
            goto finish;
        case Consonant:
	    if (state == Halant && (script != TQFont::Sinhala || uc[pos-1].unicode() == 0x200d /* ZWJ */))
                break;
            goto finish;
        case Halant:
        ...
        case Matra:
            ...
            if (script == TQFont::Bengali && uc[pos].unicode() == 0x9be && uc[pos-1].unicode() == 0x985)
                break;
...
static bool indic_shape(TQShaperItem *item)
{
...
{
    int end = from + len;
    const TQChar *uc = text.unicode() + from;
    attributes += from;
    int i = 0;
    while (i < len) {
        bool invalid;
...
static bool tibetan_shape_syllable(TQOpenType *openType, TQShaperItem *item, bool invalid)
{
    Q_UNUSED(openType)
    int len = item->length;
    ...
    const TQChar *str = item->string->unicode() + item->from;
    if (invalid) {
        *reordered.data() = 0x25cc;
...
static bool tibetan_shape(TQShaperItem *item)
{
...
static bool hangul_shape_syllable(TQOpenType *openType, TQShaperItem *item)
{
    Q_UNUSED(openType)
    const TQChar *ch = item->string->unicode() + item->from;
...
{
    Q_ASSERT(item->script == TQFont::Hangul);
