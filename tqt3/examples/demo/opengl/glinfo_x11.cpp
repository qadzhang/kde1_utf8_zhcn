...
static void screenInfo( Display *dpy, int scrnum, TQString &infotext )
{
    ...
    infotext += TQString( serverExtensions ).replace( ' ', '\n' ) + "\n\n";
    ...
    infotext += TQString( clientExtensions ).replace( ' ', '\n' ) + "\n\n";

    infotext.sprintf( "%sGLX extensions:\n", infotext.ascii() );

    infotext += TQString( glxExtensions ).replace( ' ', '\n' ) + "\n\n";
...
static void visualInfo( Display *dpy, int scrnum, TQString &configs )
{
    TQString str;
