...
{
    TQColor c = TQColorDialog::getColor( currentStyle.color, this, "editor_getcolor_dlg" );
    if ( c.isValid() ) {
...
void PreferencesBase::updatePreview()
{
    ...
    TQPalette pal = editPreview->palette();
    pal.setColor( TQPalette::Active, TQColorGroup::Text, currentStyle.color );
    pal.setColor( TQPalette::Active, TQColorGroup::Foreground, currentStyle.color );
...
void PreferencesBase::elementChanged( const TQString &element )
{
    if ( !currentElement.isEmpty() ) {
	styles.remove( currentElement );
	styles.insert( currentElement, currentStyle );
...
void PreferencesBase::familyChanged( const TQString &f )
{
    TQString oldFamily = currentStyle.font.family();
    currentStyle.font.setFamily( f );
    if ( currentElement == "Standard" ) {
	for ( TQMap<TQString, ConfigStyle>::Iterator it = styles.begin(); it != styles.end(); ++it ) {
...
void PreferencesBase::setColorPixmap( const TQColor &c )
{
    TQPixmap pm( 20, 20 );
    pm.fill( c );
...
void PreferencesBase::setPath( const TQString &p )
{
    path = p;
}
