...
struct SPair {
    SPair( const char* n, const TQString& v ) : name(n), value(v) {};
    const char* name;
    const TQString& value;
};
struct XPair {
    XPair( const char* n, const TQStringList& v, const char* s = "," ) : name(n), value(v), sep(s) {};
    const char* name;
    const TQStringList& value;
    const char* sep;
};

// void streamSPair( TQTextStream &strm, const char *n, const TQString &s )

// Streaming operators for property Pairs ---------------------------
TQTextStream &operator<<( TQTextStream &strm, const TPair &prop )
{
    switch( prop.value ) {
    case _False:
...
*/
TQTextStream &operator<<( TQTextStream &strm, const EPair &prop )
{
    strm << prop.name << prop.value << "\"";
    return strm;
}

TQTextStream &operator<<( TQTextStream &strm, const LPair &prop )
{
    strm << prop.name << prop.value << "\"";
    return strm;
}

TQTextStream &operator<<( TQTextStream &strm, const SPair &prop )
{
    if ( !prop.value.isEmpty() )
	strm << prop.name << TQString(prop.value).remove("\"") << "\"";
    return strm;
}

TQTextStream &operator<<( TQTextStream &strm, const XPair &prop )
{
    if ( !prop.value.isEmpty() ) {
...
TQTextStream &operator<<( TQTextStream &strm, const VCCLCompilerTool &tool )
{
    strm << _begTool3;
