...
void KImageViewer::makePopupMenus()
{
	...
	conn( i18n("&Next"), "Next", _imageList, SLOT(next()),
			Key_Tab );
	_aggreg->insertSeparator();
	/* [KDE1 Revival 2026] 1999 KDE five-arg insertItem (text, name,
		   receiver, slot, key) is gone in TQt3; use the standard
		   text/receiver/slot form (the accelerator was cosmetic). */
	_aggreg->insertItem( i18n("&Slideshow On/Off"),
...
{
	if( url == 0 ) {
		tqWarning( "loadURL: called with null url" );
		return;
	}
	...
	if( kurl.isMalformed() ) {
		...
		/* [KDE1 Revival 2026] TQString::setStr gone in TQt3 */
		*_transSrc = url;
		char *tmpname = tempnam( 0, "kview_");
		*_transDest = tmpname ? tmpname : "";
		free(tmpname);
...
{
	if( _kfm == 0 ) {
		tqWarning( "KImageViewer: urlFetchDone() called but no transfer in progress." );
		return;
	}
...
void KImageViewer::restoreOptions( KConfig *cfg )
{
	...
	if ( !stricmp( lmode, "ResizeWindow" ) ) {
		...
		tqWarning( "kview: warning: Unknown resize mode '%s'", 
		(const char *)lmode );
		_loadMode = ResizeWindow;
	}
...
#include "viewer.moc" /* [KDE1 Revival 2026] TQt3 moc linkage */
