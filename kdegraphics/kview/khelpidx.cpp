...
bool KHelpIndex::readIndex( const char *path )
{
	...
	if ( _index == 0 ) {
		...
		if ( _index == 0 ) {
			tqWarning( "KHelpIndex::readIndex: Out of memory" );
			return false;
		}
	...
	if( !file.exists() || !file.open( IO_ReadOnly ) ) {
		tqWarning( "KHelpIndex::readIndex: Can't read %s",
			path );
		return false;
	}
...
{
	if( !valid() ) {
		tqWarning( "KHelpIndex::invoke: Attempt to invoke"
			" invalid object." );
		return false;
	}
...
#include "khelpidx.moc" /* [KDE1 Revival 2026] TQt3 moc linkage */
