	...
	bool max )
{
	...
	if( loadOK ) {
		...
		tqWarning( "Couldn't open %s", realfile.data() );
		setStatus( BadPath );
	}
...
{
	if( urls == 0 ) {
		tqWarning( "KImageCanvas::save called with null url" );
	}
...
static QString loadStdin()
{
	...
	/* [KDE1 Revival 2026] Qt1 QString::data() writable buffer is gone in
	   TQt3; use a plain char buffer with tmpnam then wrap it. */
...
#include "canvas.moc" /* [KDE1 Revival 2026] TQt3 moc linkage */
