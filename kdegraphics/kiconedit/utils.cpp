...
void setupImageHandlers()
{
  ...
  tqWarning("Initializing formats");
...
bool copyFile(const QString &src, const QString &dest)
{
  ...
  tqWarning("Size: %u", src_size);

  if ( f_src.open(IO_ReadOnly) )
  {    // file opened successfully      
    if ( !f_dest.open(IO_WriteOnly) )
    {
      tqWarning(i18n("copyFile - There was an error opening destination file: %s"), dest.data());
      f_src.close();
      return false;
    }
    ...
    {
      tqWarning(i18n("copyFile - There was an error reading source file: %s"), src.data());
      f_src.close();
      f_dest.close();
      delete [] data;
      return false;
    ...
    {
      tqWarning(i18n("copyFile - There was an error writing to destination file: %s"), dest.data());
      f_src.close();
      f_dest.close();
      delete [] data;
      return false;
  ...
  tqWarning(i18n("copyFile - There was an error opening source file: %s"), src.data());
  return false;
}
...
bool removeFile(const QString &file)
{
  ...
  {
    QDir d;
    tqWarning("Removing %s", file.data());
    if(!d.remove(file.data()))
    {
      tqWarning(i18n("removeFile - There was an error removing the file: %s"), file.data());
      return false;
    }
