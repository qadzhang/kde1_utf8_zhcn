...
void KIconEdit::init()
{
  ...
  if(!windowList)
  {
    tqWarning("KIconEdit::KIconEdit() Globalizing windowlist");
  ...
  tqWarning("Grid created");
  grid->setGrid(pprops->showgrid);
  tqWarning("Grid->setGrid done");
  grid->setCellSize(pprops->gridscaling);
  tqWarning("Grid->setCellSize done");
  ...
  tqWarning("Setting tool");
  slotTools(ID_DRAW_FIND);
  tqWarning("Updating statusbar");
...
KIconEdit::~KIconEdit()
{
  tqWarning("KIconEdit - Destructor");
...
*/
  ...
  tqWarning("KIconEdit - Destructor: done");
}
...
void KIconEdit::closeEvent(QCloseEvent *e)
{
  ...
  if(!cancel)
  {
    ...
    {
      tqWarning("KIconEdit: Last in list closes the door...");
      kapp->quit();
    }
  }
  else
...
void KIconEdit::resizeEvent( QResizeEvent * )
{
  ...
  //tqWarning("KIconEdit::resizeEvent()");
...
{
  tqWarning("KIconEdit::saveGoingDownStatus");
  if( grid->isModified() )
  {
    tqWarning("Saving backup for modified file %s", icon->url().data());
    icon->saveBackup(&grid->image());
  }
  else
    icon->cleanup();
  tqWarning("KIconEdit::saveGoingDownStatus - done");
}

// this is for exit by request of the session manager
void KIconEdit::saveProperties(KConfig *config )
{
  tqWarning("KIconEdit::saveProperties");
...
{
  tqWarning("KIconEdit::readProperties");
...
bool KIconEdit::setupMenuBar()
{
  ...
  tqWarning("setupMenuBar - read settings");
...
bool KIconEdit::setupToolBar()
{
  ...
  /* [KDE1 Revival 2026] TQt3 QWhatsThis is a static utility class */
  QWidget *btwhat = QWhatsThis::whatsThisButton(toolbar);
...
*/
  ...
  tqWarning("setupToolBar - done");
  return true;
}

bool KIconEdit::setupDrawToolBar()
{
  tqWarning("setupDrawToolBar");
  Properties *pprops = props(this);
  ...
  tqWarning("setupDrawToolBar - done");
  return true;
}

bool KIconEdit::setupPaletteToolBar()
{
  tqWarning("setupPaletteToolBar");
  //Properties *pprops = props(this);
...
void KIconEdit::addRecent(const char *filename)
{
  ...
  //tqWarning("addRecent - checking %s", filename);
  Properties *pprops = props(this);
  if(str.isEmpty() || pprops->recentlist->contains(filename))
    return;
  //tqWarning("addRecent - adding %s", filename);
...
#include "kiconedit.moc" /* [KDE1 Revival 2026] TQt3 moc linkage */
