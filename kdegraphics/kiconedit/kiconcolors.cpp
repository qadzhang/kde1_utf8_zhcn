...
{
  tqWarning("KDrawColors - constructor");
  setCellSize(17);
  setGrid(true);
  setGridState(KColorGrid::Shaded);
  selected = 0;
  tqWarning("KDrawColors - constructor - done");
}
...
{
  tqWarning("KSysColors - constructor");
  ...
  tqWarning("KSysColors - constructor - before setColor");
  for(int i = 0; i < numcells; i++)
    setColor(i, iconpalette[i]|OPAQUE_MASK);
  tqWarning("KSysColors - constructor - done");
}

KCustomColors::KCustomColors(QWidget *parent) : KDrawColors(parent)
{
  tqWarning("KCustomColors - constructor");
  setNumRows(3);
  setNumCols(6);
  ...
  for(int i = 0; i < numRows()*numCols(); i++)
    freecells[i] = true;
  tqWarning("KCustomColors - constructor - done");
}
...
void KCustomColors::addColor(uint c)
{
  if(!contains(c))
  {
    ...
    if(f != -1)
    {
      ...
      if(!color.isValid())
      {
        tqWarning("KCustomColors::addColor: Not a valid color: %u", c);
        return;
      }
      //tqWarning("KCustomColors::addColor: Adding color: %u - %d %d %d", c, qRed(c), qGreen(c), qBlue(c));
      setColor(f, c);
      freecells[f] = false;
    }
  }
...
#include "kiconcolors.moc" /* [KDE1 Revival 2026] TQt3 moc linkage */
