...
{
  //tqWarning("KIconFileView - constructor");
  width_max = 100;
  width_length = 0;
...
{
  tqWarning("KIconFileView - destructor");
}
...
{ 
    tqWarning("highlightItem %d %d", row, col);
...
void KIconFileView::paintCell( QPainter *p, int row, int col)
{
    ...
    //tqWarning("paintCell: %s", text(index));
...
{
//tqWarning("insertItem: %s", i->fileName());
    QPixmap *pix;
    QString path;
...
#include "kiconfileview.moc" /* [KDE1 Revival 2026] TQt3 moc linkage */
