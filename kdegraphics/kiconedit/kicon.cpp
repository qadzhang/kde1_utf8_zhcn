...
void KIcon::open(const QImage *image, const char *xpm)
{
  ...
  if(filename.isEmpty())
    return;
  tqWarning("KIcon::open %s", filename.data());
  if(kfm != 0L)
  {
  ...
  tqWarning("KIcon::open - checked for absolute path");

  KURL _turl(filename.data());
  if(_turl.isMalformed()) // try to see if it is a relative filename
  {
    tqWarning("KIcon::open (malformed) %s", filename.data());
  ...
  tqWarning("KIcon::open (not malformed) %s", filename.data());
  ...
  if(r)
  {
    tqWarning("KIcon: Found recover file");
    ...
    {
      tqWarning("Opening recover file for: %s", filename.data());
    }
  }
  else
  {
    ...
    if(t)
      delete t;
    //tqWarning("KIcon: tempSaveName: %s", tmp.data());

    tqWarning("isLocalFile   : %d", _turl.isLocalFile());
    tqWarning("hasSubProtocol: %d", _turl.hasSubProtocol());
    tqWarning("Path:         : %s", _turl.path());
    tqWarning("URL:          : %s", _turl.url().data());

    if(_turl.isLocalFile() && !_turl.hasSubProtocol())
    {
      tqWarning("KIcon::open local: %s", _turl.url().data());
    ...
    {
      tqWarning("KIcon::open non-local: %s %s", _turl.path(), tmp.data());
      if(kfm != 0L)
      {
      ...
      tqWarning("KIcon::open - before download: %s", tmp.data());
      removeFile(_backup);
      ...
      tqWarning("KIcon::open - after download: %s", tmp.data());

      closeKFM();
      tqWarning("KIcon::open - after closeKFM");
    } // remote
    if(!QFile::exists(tmp))
    {
    ...
    tqWarning("KIcon::open - backup created");
  } // not recovered
  ...
  tqWarning("KIcon::open - Before loading image");
  ...
  tqWarning("KIcon::open - Image loaded");
  unlock(f);
  f = fs;
  _backup = tmp;
  _url = _turl.url();

  tqWarning("KIcon: _backup: %s", _backup.data());
  tqWarning("KIcon: _url: %s", _url.data());
  emit loaded(img);
  emit newname(_url.data());
  tqWarning("KIcon::open - done");
}
...
void KIcon::checkUnNamedBackup(const QImage *image)
{
  ...
  if(r)
  {
    ...
    if(KMsgBox::yesNo((QWidget*)parent(), i18n("Information"), msg.data()) == 1)
    {
      tqWarning("Opening recover file for: %s", tmp.data());
      if(!img->load(tmp.data()))
      {
        QString msg = i18n("KIcon: There was a QT error loading:\n");
        msg += tmp;
      ...
      tqWarning("KIcon::checkForUnNamed - Image loaded");
      _backup = tmp;
      emit loaded(img);
...
void KIcon::save(const QImage *image, const char *filename)
{
  ...
  if(iio.write())
  {
    tqWarning("KIcon::save - backup image written: %s", str.data());
    KURL _turl(filename);
...
*/
  ...
  else // if(iio.write(...
  {
    ...
    msg += "\n";
    KMsgBox::message((QWidget*)parent(), i18n("ERROR"), msg, KMsgBox::EXCLAMATION);
    tqWarning("KIcon::save - %s", msg.data());
  }
  delete tmp;
  tqWarning("KIcon::save - done");
}

void KIcon::saveAs(const QImage *image)
{
  tqWarning("KIcon::saveAs");
  if(kfm != 0L)
  {
    KMsgBox::message((QWidget*)parent(), i18n("Warning"),
      i18n("We've got a KFM job running.\nPlease wait."), KMsgBox::STOP);
...
FILE *KIcon::lock(const char *file)
{
  ...
  if (!fs) 
  {
    tqWarning("Cannot open file `%s': %s", file, 
	  strerror(errno));
    return 0;
  }
  ...
  if (rc < 0)
  {
    tqWarning("Cannot lock `%s': %s (%d)", file,
  ...
  tqWarning("Locked %d %s", fileno(fs), file);
  return fs;
}
...
bool KIcon::unlock(FILE *fs)
{
...
#if HAVE_FLOCK
  tqWarning("Using flock");
  rc = flock(fileno(fs), LOCK_UN);
#else
  tqWarning("Using fcntl");
  rc = fcntl(fileno(fs), F_SETLK, &fl);
  ...
  if(rc == 0)
  {
    tqWarning("Unlocked : %d", fileno(fs));
    fclose(fs);
    fs = 0;
    return true;
  }
  tqWarning("Cannot unlock descriptor %d: %s (%d)", fileno(fs), strerror(errno), errno);
...
#include "kicon.moc" /* [KDE1 Revival 2026] TQt3 moc linkage */
