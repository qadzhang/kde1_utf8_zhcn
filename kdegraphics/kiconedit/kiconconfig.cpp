...
{
  tqWarning("KTemplateConfig constructor");
  ...
  tqWarning("KTemplateConfig constructor - done");
}

void KTemplateConfig::saveSettings()
{
  tqWarning("KTemplateConfig::saveSettings");
  ...
  tqWarning("KTemplateConfig::saveSettings - done");

}

void KTemplateConfig::checkSelection(int)
{
  tqWarning("KTemplateConfig::checkSelection");
  ...
  tqWarning("KTemplateConfig::checkSelection - done");
}
...
{
  tqWarning("KBackgroundConfig - constructor");
  ...
  if(pix.isNull())
  {
    tqWarning("BGPIX: %s not valid!", pixpath.data());
  ...
  tqWarning("KBackgroundConfig - constructor done");
}
...
{
  tqWarning("KToolBarConfig - constructor");
  ...
  tqWarning("KToolBarConfig - constructor done");
}
...
{
  tqWarning("KToolBarConfig::saveSettings");
  ...
  tqWarning("KToolBarConfig::saveSettings - done");
}
...
{
  tqWarning("KMiscConfig - constructor");
  ...
  tqWarning("KMiscConfig - constructor done");
}
...
{
  tqWarning("KMiscConfig::saveSettings");
  pprops->pastetransparent = pastemode;
  pprops->showrulers = showrulers;
  tqWarning("KMiscConfig::saveSettings - done");
}
...
{
  pprops = KIconEditProperties::getProperties(parent);
  tqWarning("KIconConfig - constructor");
  setCaption("Configure ");
  //keys = k; //new KAccel(parent);
  //dict = new QDict<KKeyEntry>( k->keyDict() );
  //tqWarning("Keys: %u", k->keyDict().count());
  //tqWarning("Keys: %u", dict->count());
  //CHECK_PTR(dict);
  pprops->keydict = pprops->keys->keyDict();
  tqWarning("Keys: %u", pprops->keydict.count());
  ...
  tab = new QTab;            // create a QTab to hold the tab data
  tab->setText( i18n("Miscellaneous") );
  tab->setEnabled( true );
  addTab( tab );
  p = new KWizardPage;
  p->w = misc;
  ...
  tqWarning("KIconConfig - constructor - done");
}
...
{
  tqWarning("KIconEditConfig::saveSettings");

  pprops->keys->setKeyDict(pprops->keydict);
  tqWarning("KIconEditConfig::saveSettings - keys saved");
  temps->saveSettings();
  backs->saveSettings();
  misc->saveSettings();
  toolbars->saveSettings();
  tqWarning("KIconEditConfig::saveSettings - done");
  accept();
}



#include "kiconconfig.moc" /* [KDE1 Revival 2026] TQt3 moc linkage */
