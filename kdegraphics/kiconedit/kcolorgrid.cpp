...
void KColorArray::remove(int idx)
{
  ...
  if(idx >= (int)size())
  {
    tqWarning("KColorArray::remove: Index %d out of range", idx);
    return;
  }
  ...
  for(int i = idx; i < oldsize-1; i++)
    at(i) = tmp[i+1];
  //tqWarning("KColorArray::remove() %u\t-\tsize: %d", at(idx), size());
}
...
void KColorArray::append(uint c)
{
  resize(size()+1);
  at(size()-1) = c;
  //tqWarning("KColorArray::append() %u\t-\tsize: %d", c, size());
}

uint KColorArray::closestMatch(uint color)
{
  //tqWarning("KColorArray: %u", c);
  uint c = color & ~OPAQUE_MASK, d = 0xffffff, t;
  //tqWarning("KColorArray: %u", c);
  //tqWarning("KColorArray: %u", c|OPAQUE_MASK);
  uint cb = c;
...
KColorGrid::KColorGrid(QWidget *parent, const char *name, int space)
 : QWidget(parent, name)
{
  //tqWarning("KColorGrid - constructor");
  s = space;
  ...
  //tqWarning("KColorGrid - constructor - done");
}
/*
...
{
  //tqWarning("KColorGrid::paintEvent");
  ...
  //tqWarning("%d x %d  -  row: %d x %d", urect.x(), urect.y(), urect.width(), urect.height());
  //tqWarning("col: %d -> %d  -  row: %d -> %d", firstcol, lastcol, firstrow, lastrow);
...
{
  //tqWarning("resize: width:  %d - total: %d", width(), totalwidth);
  //tqWarning("resize: height: %d - total: %d", height(), totalheight);
}
*/
...
const QRect KColorGrid::viewRect()
{
  ...
  //tqWarning("viewRect - %d x %d - %d x %d", x, y, w, h);
  return r;
}

void KColorGrid::setNumRows(int n)
{
  //tqWarning("setNumRows");
  if(n < 0 || n == rows)
    return;
  ...
  //tqWarning("setNumRows() - gridcolors: %d  size: %d", gridcolors.size(), numCols()*numRows());
}

void KColorGrid::setNumCols(int n)
{
  //tqWarning("setNumCols");
  if(n < 0)
    return;
  int on = numCols();
  KColorArray gc(gridcolors);
  gc.detach();
  //tqWarning("gc size: %d numrows: %d", gc.size(), numRows());
  gridcolors.resize(n*numRows());
  cols = n;

  totalwidth = (n * cellsize) + 1;
  resize(totalwidth, totalheight);
  //tqWarning("numRows: %d", numRows());
  //tqWarning("gridcolor: %d  grid: %d", gridcolors.size(), numRows()*numCols());
  if(numRows() == 0)
    return;
  ...
  for(int i = 0; i < numRows(); i++)
  {
    for(int j = 0; j < on; j++)
    {
      //tqWarning("row %d , col %d", i, j);
      if(on <= n)
      {
        //tqWarning("%d %d", (i*numCols())+j, (i*on)+j);
  ...
  //tqWarning("setNumCols() - gridcolors: %d  size: %d", gridcolors.size(), numCols()*numRows());
}
...
void KColorGrid::setColor( int colNum, uint col, bool update )
{
  ...
  //tqWarning("KColorGrid::setColor - before adding");
  if(!numcolors.contains(col))
  {
    //tqWarning("KColorGrid::setColor() - adding %u - %d %d %d",
    //  col, qRed(col), qGreen(col), qBlue(col));
    numcolors.append(col);
    //tqWarning("KColorGrid::setColor() - adding done %d", numcolors.size()-1); 
    //numcolors++;
    emit addingcolor(col);
  }

  //tqWarning("KColorGrid::setColor - before removing");
  if(!gridcolors.contains(oldcolor))
  {
    int idx = numcolors.find(oldcolor);
    if(idx != -1)
    {
      //tqWarning("KColorGrid::setColor() - removing %u - %d %d %d",
      //  oldcolor, qRed(oldcolor), qGreen(oldcolor), qBlue(oldcolor));
      numcolors.remove(idx);
      //tqWarning("KColorGrid::setColor() - removing done"); 
      emit colorschanged(numcolors.size(), numcolors.data());
    }
    //numcolors--;
  }

  //tqWarning("KColorGrid::setColor - before updateCell");
  repaint((colNum%numCols())*cellsize,(colNum/numCols())*cellsize,  cellsize, cellsize);
  //updateCell( colNum/numCols(), colNum%numCols(), false );
  //tqWarning("KColorGrid::setColor - after updateCell");
}

void KColorGrid::updateCell( int row, int col, bool erase )
{
    //tqWarning("updateCell - before repaint");
  QWMatrix matrix;
  QPainter p;
  p.begin( this );
...
#include "kcolorgrid.moc" /* [KDE1 Revival 2026] TQt3 moc linkage */
