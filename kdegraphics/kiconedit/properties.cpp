...
{
  instances++;
  //tqWarning("KIconEditProperties: instances %d", instances);
  if(pprops)
    return;

  tqWarning("KIconEditProperties: reading properties");
  ...
  tqWarning("Read %i recent files", n);
  ...
  tqWarning("Reading bgmode: %d", (int)pprops->backgroundmode);
  ...
  tqWarning("BackgroundPixmap: %s", pprops->backgroundpixmap.data());
...
KIconEditProperties::~KIconEditProperties()
{
  ...
  if(instances == 0)
  {
    tqWarning("KIconEditProperties: Deleting properties");
    if(pprops->recentlist)
      delete pprops->recentlist;
    delete pprops;
    tqWarning("KIconEditProperties: Deleted properties");
  }
}
...
void KIconEditProperties::saveProperties(QWidget *parent)
{
  ...
  config->setGroup( "Files" );
  tqWarning("Writing %d recent files", pprops->recentlist->count());
  ...
  tqWarning("Writing bgmode: %d", (int)pprops->backgroundmode);
...
#include "properties.moc" /* [KDE1 Revival 2026] TQt3 moc linkage */
