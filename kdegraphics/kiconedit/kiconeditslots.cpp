...
{
  //tqWarning("KIconTools::addColors()");
  customcolors->clear();
  for(uint i = 0; i < n; i++)
    addColor(c[i]);
}
...
{
  tqWarning("KIconEdit::openNewWin() - %s", url);
...
void KIconEdit::slotNew()
{
  ...
  if(!cancel)
  {
    ...
    if(n.exec())
    {
      ...
      tqWarning("Size: %d x %d", s.width(), s.height());
      grid->setSize(s);
...
{
  tqWarning("KIconEdit: Closing %u windows", memberList->count());
  KIconEdit *ki = 0L;
...
{
  tqWarning("KIconEdit: Saving %s", icon->url().data());
  icon->save(&grid->image());
}
...
{
  tqWarning("Opening recent file: %d", id);
  Properties *pprops = props(this);
  tqWarning("Recent: %s", pprops->recentlist->at((uint)id));
...
{
  tqWarning("KIconEdit::slotDropEvent");
  ...
  for ( s = list.first(); s != 0L; s = list.next() )
  {
    ...
    tqWarning("KIconEdit:slotDropEvent - %s", s);
    //s == list.getFirst();
...
void KIconEdit::slotQDropEvent( QDropEvent *e )
{
  ...
  else if(QUrlDrag::decode( e, list ) )
  {
    ...
    {
      // Load the first file in this window
      tqWarning("KIconEdit:slotQDropEvent - %s", s);
      //s == list.getFirst();
      if (!grid->isModified() && !loadedinthis) 
      {
        icon->open( &grid->image(), s );
...
{
  tqWarning("Got QDragLeaveEvent!");
  gridview->setAcceptDrop(false);
}

void KIconEdit::slotQDragEnterEvent( QDragEnterEvent *e )
{
  tqWarning("Got QDragEnterEvent!");
