...
{
  //tqWarning("KIconFileBase - constructor");
...
KIconFileBase::~KIconFileBase()
{
  tqWarning("KIconFileBase - destructor");
...
bool KIconFileBase::checkXVFile(const KFileInfo *i, const char *filepath, QPixmap *pixmap)
{
   ...
   if( !fi.exists() )
   {
     ...
     if(!pixmap->save(path.data(), "XV"))
       tqWarning("Error saving XV thumbnail %s", path.data());
   }
   ...
   if( fi.exists() && !hasimage )  
   {
     ...
     {
       tqWarning("Error loading XV thumbnail %s", path.data());
       return false;
     }
   }
...
#include "kiconfilebase.moc" /* [KDE1 Revival 2026] TQt3 moc linkage */
