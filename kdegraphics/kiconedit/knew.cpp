...
KIconTemplateContainer::KIconTemplateContainer() : QObject()
{
  QString path;
  instances++;
  tqWarning("KIconTemplateContainer: instances %d", instances);
  if(templatelist)
    return;

  tqWarning("KIconTemplateContainer: Creating templates");
...
KIconTemplateContainer::~KIconTemplateContainer()
{
  ...
  if(instances == 0)
  {
    tqWarning("KIconTemplateContainer: Deleting templates");
    templatelist->clear();
    delete templatelist;
    tqWarning("KIconTemplateContainer: Deleted templates");
  }
}
...
KIconListBoxItem::KIconListBoxItem( KIconTemplate *t )
   : QListBoxItem()
{
  //tqWarning("KIconListBoxItem - %s, %s", t->path.data(), t->title.data());
  icontemplate = t;
  pm.load(t->path.data());
...
NewSelect::~NewSelect()
{
  ...
  //tqWarning("NewSelect - destructor");
  delete ml;
  delete l;
/*
  delete rbscratch;
  ...
  delete grp;
*/
  tqWarning("NewSelect - destructor: done");
}

void NewSelect::buttonClicked(int id)
{
  //tqWarning("Button: %d", id);
...
NewFromTemplate::NewFromTemplate(QWidget *parent) : QWidget( parent )
{
  ...
  tqWarning("NewFromTemplate: templates %u", tlist->count());
...
{
  tqWarning("NewFromTemplate - destructor");
  delete ml;
  delete l;
  //delete grp;
  tqWarning("NewFromTemplate - destructor: done");
}
...
{
  tqWarning("KNewIcon - constructor");
  if(isTopLevel())
    tqWarning("isTopLevel");
  else
    tqWarning("not TopLevel");
  //hide();
...
KNewIcon::~KNewIcon()
{
  tqWarning("KNewIcon - destructor");
  delete select;
  delete scratch;
  delete templ;
  //delete p1;
  //delete p2;
  tqWarning("KNewIcon - destructor: done");
}
...
#include "knew.moc" /* [KDE1 Revival 2026] TQt3 moc linkage */
