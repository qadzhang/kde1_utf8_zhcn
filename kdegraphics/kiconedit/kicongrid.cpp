...
{
  tqWarning("KGridView::resizeEvent");
  setSizes();
  if(pprops->showrulers)
  {
 ...
 : KColorGrid(parent, name, 1)
{
  ...
  setupImageHandlers();
  tqWarning("Formats: %d", formats->count());
...
{
  tqWarning("KIconEditGrid - destructor: done");
}

void KIconEditGrid::paintEvent(QPaintEvent *e)
{
//tqWarning("KIconGrid::paintEvent");
...
//tqWarning("KIconGrid::paintEvent - done");
}
...
void KIconEditGrid::paintCell( QPainter *painter, int row, int col )
{
  ...
  //tqWarning("KIconEditGrid::paintCell() - done");
}
...
void KIconEditGrid::mouseMoveEvent( QMouseEvent *e )
{
  ...
  if(ispasting && !btndown && img->valid(col, row))
  {
    ...
    //tqWarning("Moving: %d x %d", insrect.width(), insrect.height());
...
/*
      ...
      //tqWarning("KIcnGrid::load: %d %%", (((y*ncols)+x)/imgsize) * 100);
      //tqWarning("KIconEditGrid::load: RGB: %d %d %d", qRed(*l), qGreen(*l), qBlue(*l));
*/
...
{
  tqWarning("KIconEditGrid::getImage");
  *image = *img;
}
...
const QImage *KIconEditGrid::clipboardImage(bool &ok)
{
  ...
  if(pix.isNull())
    tqWarning("Invalid pixmap in clipboard!");
  else
  {
    tqWarning("Valid pixmap in clipboard!");
    *image = pix;
  }
  return image;
}
...
{
  ok = false;
  /* [KDE1 Revival 2026] TQt3 TQBuffer takes a TQByteArray */
  TQByteArray clipbytes = kapp->clipboard()->text().utf8();
  TQBuffer buf(clipbytes);
...
QImage *KIconEditGrid::getSelection(bool cut)
{
  ...
  int nx = 0, ny = 0, nw = 0, nh = 0;
  rect.rect(&nx, &ny, &nw, &nh);
  tqWarning("KIconEditGrid: Selection Rect: %d %d %d %d", nx, ny, nw, nh);
...
void KIconEditGrid::editCopy(bool cut)
{
  ...
  /* [KDE1 Revival 2026] TQt3 TQBuffer takes TQByteArray; clipboard via utf8 */
  TQByteArray str;
  TQBuffer buf(str);
...
void KIconEditGrid::editPaste(bool paste)
{
  ...
  if(ok)
  {
    ...
    else if(!paste)
    {
      ispasting = true;
      cbsize = tmp->size();
      //tqWarning("insrect size: %d x %d", insrect.width(), insrect.height());
      return;
      emit newmessage(i18n("Pasting"));
    }
    else
    {
      //tqWarning("KIconEditGrid: Pasting at: %d x %d", insrect.x(), insrect.y());
...
{
  tqWarning("KIconGrid::editResize");
#if QT_VERSION >= 140
...
{
  tqWarning("::setSize: %d x %d", s.width(), s.height());
...
void KIconEditGrid::createCursors()
{
  ...
  if(pix.isNull())
  {
    ...
    tqWarning("KIconEditGrid: Error loading pixmap: paintbrush.xpm"); 
  }
  else
  {
  ...
  if(pix.isNull())
  {
    ...
    tqWarning("KIconEditGrid: Error loading pixmap: flood.xpm"); 
  }
  else
  {
    mask = pix.createHeuristicMask();
  ...
  if(pix.isNull())
  {
    ...
    tqWarning("KIconEditGrid: Error loading pixmap: aim.xpm"); 
  }
  else
  {
    mask = pix.createHeuristicMask();
...
#include "kicongrid.moc" /* [KDE1 Revival 2026] TQt3 moc linkage */
