    ...
    : QLabel( "DragSource", parent, name )
{
  ...
  else tqWarning("Unknown datatype: %s", dragtype);
}
...
{
  tqWarning("KDragSource::mousePressEvent");

  if(!ok) return;

  if(type.left(6) == "image/")
  {
    tqWarning("Type: image");
    QImage img;
    emit getimage(&img);
    if(img.isNull())
      tqWarning("Bad image");
    else
    {
      ...
      tqWarning("KDragSource::mousePressEvent - before dragCopy");
#if QT_VERSION > 140
  ...
  else tqWarning("Unknown datatype: %s", type.data());

  tqWarning("KDragSource::mousePressEvent - done");
}
...
#include "kdragsource.moc" /* [KDE1 Revival 2026] TQt3 moc linkage */
