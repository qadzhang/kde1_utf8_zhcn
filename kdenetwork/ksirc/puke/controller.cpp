...
#include "playout.h"
#include <config.h>  /* TQt3 migration: build-tree relative paths replaced by include-path config.h */
#include "../objFinder.h"
...
{
  int cfd;
  socklen_t len = 0;  /* TQt3 migration: KSIZE_T was Qt1-era, use socklen_t */
  struct sockaddr_un unix_addr;
...
//  tqDebug("Fetching new  widget, type: %d, parent: %d objname: %s", iType, iParent, name);
...
//    tqWarning("new  widget: %d with wc: %p and handle: %p", pm->iArg, wc, handle);
